...
func TestIntegerArithmethic(t *testing.T) {
	tests := []compilerTestCase{
		{
			input:             "1 + 2",
			expectedConstants: []interface{}{1, 2},
...
func testInstructions(expected []code.Instructions, actual code.Instructions) error {
	concatted := concatInstuctions(expected)
...
func concatInstuctions(s []code.Instructions) code.Instructions {
...
func testConstants(t *testing.T, expected []interface{}, actual []object.Object) error {
	if len(expected) != len(actual) {
		return fmt.Errorf("wrong number of constants. got=%d, want=%d", len(expected), len(actual))
	}
...
func testIntegerObject(expected int64, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("object has wrong value. got=%d, want=%d", result.Value, actual)
	}
